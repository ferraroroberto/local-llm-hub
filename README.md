...
OpenAI-shape at `POST /v1/images/generations`, from either Google Imagen (via
the Antigravity CLI) or FLUX.1 [dev] running locally on the GPU.
...
- **`flux1_local`** (alias `flux`) — the same `POST /v1/images/generations`
  contract, served **entirely locally** by **FLUX.1 [dev]** (fp8) on ComfyUI,
  on tower's RTX 5060 Ti. Additive rather than a replacement for
  `gemini_image`: no subscription, no network call, and no content policy
...
openClaw / anthropic SDK / openai SDK / curl
   ...
   │    POST /v1/images/generations (flux1_local) → ComfyUI :8188        │
   │      (local FLUX.1 [dev] fp8, on_demand; same b64_json shape)       │
...
│   ├── install_comfyui.py       # ComfyUI clone + isolated venv + torch cu130 (sm_120 verified) → vendor/comfyui/ (#492)
...
└── docs/
    ...
    ├── image-generation.md       # Imagen via agy + local FLUX via ComfyUI
