...
# ---- Image generation (OpenAI /v1/images/generations shape) ----
...
def images_generations(req: ImagesGenerationRequest, request: Request) -> JSONResponse:
    ...
    The only image backend the hub can reach is Google's Imagen, exposed as
    an agentic tool inside `agy` (there is no Nano Banana picker model — see
    issue #114). So this routes exclusively to gemini rows flagged
    ``image_gen``; every other backend is text-only and 400s. The call lands
    in the observability ring like other hub traffic.
    """
    model = _resolve(req.model)
