...
start "claude-local-calls - gemma4-e4b"      cmd /k .venv\Scripts\python.exe -m src.run_backend gemma4_e4b
start "claude-local-calls - gemma4-26b-a4b"  cmd /k .venv\Scripts\python.exe -m src.run_backend gemma4_26b
start "claude-local-calls - whisper"         cmd /k .venv\Scripts\python.exe -m src.run_backend whisper

echo Launched hub + qwen + glm + gemma4-e4b + gemma4-26b-a4b + whisper in separate windows.
